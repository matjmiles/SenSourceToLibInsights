...
function Get-ZoneTrafficData {
    ...
    try {
        ...
        if ($Response.results -and $Response.results.Count -gt 0) {
            ...
            Write-Host "  ⚠️ No data returned for this zone" -ForegroundColor Yellow
            return $null
        }
    }
    catch {
...
function Create-ZoneSpringshareCSV {
    ...
    if ($DailyTotals.Count -eq 0) {
        Write-Host "  ⚠️ No valid daily records found" -ForegroundColor Yellow
        return $null
    }
...
Write-Host "`nStep 2: Getting zones list" -ForegroundColor Yellow
...
Write-Host "`nStep 3: Processing each zone" -ForegroundColor Yellow
...
foreach ($zone in $Zones) {
    Write-Host "`n🔍 Processing Zone:" -ForegroundColor Cyan
    ...
    if (-not $SensorName) {
        ...
        Write-Host "   ⚠️ Unknown sensor, using generic name" -ForegroundColor Yellow
    }
