# VEA Multi-Sensor Data Extractor
# Extracts data for each sensor individually and creates separate Springshare CSV files
...
# Sensor list from your API response
$Sensors = @(
...
function Get-SensorData {
    ...
    $QueryParams = @{
        ...
        "sensorId" = $SensorId  # Add sensor filter
    }
...
Write-Host "`nStep 2: Processing each sensor..." -ForegroundColor Yellow
...
foreach ($sensor in $Sensors) {
    $ProcessedCount++
    Write-Host "`n[$ProcessedCount/$TotalSensors] Processing: $($sensor.Name)" -ForegroundColor Cyan
...
# List created files
Write-Host "`nCreated CSV files:" -ForegroundColor Cyan
